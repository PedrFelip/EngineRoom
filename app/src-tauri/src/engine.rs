...
enum SpawnKind {
    /// Sidecar by basename (e.g. "stockfish" or "stockfish-lite"). Tauri
    /// resolves the platform binary from `binaries/<name>-<triple>` next to
    /// the main exe at runtime, in both dev and bundled builds.
    Sidecar(String),
    /// Absolute path to an arbitrary engine binary on the host filesystem.
    Path(String),
}
...
fn spawn_engine(
    ...
    match kind {
        SpawnKind::Sidecar(name) => {
            let command = app
                .shell()
                .sidecar(&name)
                .map_err(|e| format!("Não foi possível localizar o Stockfish embarcado ({name}): {e}"))?;
...
pub fn engine_spawn(
        ...
        .filter(|s| !s.is_empty())
    {
        ...
        None => SpawnKind::Sidecar(sidecar.unwrap_or_else(|| SIDECAR.to_string())),
    };
