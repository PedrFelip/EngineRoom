...
describe('analyzeGame', () => {
  ...
  it('com keepAlive=true não envia `quit` ao final (deixa a engine viva)', async () => {
    const sent: string[] = []
    let lineCb: ((line: string) => void) | null = null
    const port: EnginePort = {
      send(cmd) {
        ...
        if (c === 'uci') lineCb?.('uciok')
          ...
          lineCb?.('info depth 20 multipv 1 score cp 0 pv e2e4')
          lineCb?.('bestmove e2e4')
        }
      },
      onLine(handler) {
    ...
    expect(sent).not.toContain('quit')
  })

  it('sem keepAlive envia `quit` ao final (comportamento padrão)', async () => {
    const sent: string[] = []
    let lineCb: ((line: string) => void) | null = null
    const port: EnginePort = {
      send(cmd) {
    ...
    await analyzeGame('1. e4 e5', { mode: 'depth', depth: 20 }, port)
