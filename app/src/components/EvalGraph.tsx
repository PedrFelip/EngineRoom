...
export default function EvalGraph({ winPcts, currentPly, onSelect }: EvalGraphProps) {
  ...
  const ready = w > 0 && n >= 2;
  const curvePts = ready ? winPcts.map((wp, i) => `${x(i).toFixed(1)},${y(wp).toFixed(1)}`) : [];
    ...
    <div ref={ref} className="w-full">
      {ready && (
        ...
        >
          ...
          <line x1={0} y1={mid} x2={w} y2={mid} stroke="var(--color-edge)" strokeWidth={1} />
